...
router.get('/',
  ...
  [
    ...
    query('status').optional({ checkFalsy: true }).isIn(VALID_STATUS).withMessage('status không hợp lệ.'),
    query('industryId').optional({ checkFalsy: true }).isInt({ min: 1 }),
    query('assignedTo').optional({ checkFalsy: true }).isInt({ min: 1 }),
  ],
  validate,
  ctrl.list
);
...
router.post('/',
  ...
  [
    ...
    body('taxCode').optional({ checkFalsy: true }).isString().trim(),
...
router.put('/:id',
  ...
  [
    param('id').isInt({ min: 1 }),
    body('companyName').optional({ checkFalsy: true }).notEmpty().withMessage('Tên doanh nghiệp không được để trống.'),
    body('industryId').optional({ checkFalsy: true }).isInt({ min: 1 }),
    body('assignedTo').optional({ checkFalsy: true }).isInt({ min: 1 }),
    body('website').optional({ checkFalsy: true }).isURL().withMessage('Website URL không hợp lệ.'),
    body('representativeName').optional({ checkFalsy: true }).isString().trim(),
...
router.post('/:id/contacts',
  ...
  [
    ...
    body('phone').optional({ checkFalsy: true }).isString().trim(),
    body('email').optional({ checkFalsy: true }).isEmail().withMessage('Email không hợp lệ.'),
    body('isPrimary').optional({ checkFalsy: true }).isBoolean(),
  ],
  validate,
  ctrl.addContact
);
...
router.put('/:id/contacts/:contactId',
  ...
  [
    ...
    body('fullName').optional({ checkFalsy: true }).notEmpty(),
